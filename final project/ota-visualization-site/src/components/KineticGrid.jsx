...
const KineticGrid = ({ theme }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      /* 4. Ripple displacement (additive) */
      ...
      /* 5. Lerp point positions toward targets */
      for (let i = 0; i < pts.length; i++) {
        const p = pts[i];
      ...
      /* 7. Draw grid lines — bucketed by alpha for performance */
      ...
      const assignBucket = (midX, midY, p1, p2) => {
  ...
  return <canvas ref={canvasRef} className="kinetic-grid-canvas" />;
};
