...
We propose a mark of 85 (A2), reflecting a project that is "outstanding in some respects" and demonstrates "original, sophisticated independent thinking." Our work successfully replicated the core findings of Ota et al. (2009) [1] using a modern Bayesian framework and extended the analysis to test the underlying linguistic theory with greater precision. This demonstrates a thorough and sophisticated application of the principles learned in this course.
...
|---|---|---|
| Comprehensive | `accuracy ~ contrast_type + (1\|subject_id) + (1\|item_id)` | Replicate original design |
| Linguistic | `accuracy ~ phonological_status + (1\|subject_id) + (1\|item_id)` | Test theoretical hierarchy |
| Distinctness | `accuracy ~ phonological_distinctness_scaled + (1\|subject_id) + (1\|item_id)` | Quantify core mechanism |
...
Our work was conducted with a strong commitment to open and reproducible science. The entire analysis, processing 1,389 trials from 20 subjects, is available in a fully documented R script (`TaskA_.r`) and will be shared on a public GitHub repository. We explicitly defined and justified our priors (`normal(0, 1.5)` for fixed effects, `exponential(1)` for random effects) to ensure transparency. Furthermore, we validated our models using posterior predictive checks, which confirmed that our simulations accurately captured the distribution of the observed data. This demonstrates a "thorough understanding of Open Scholarship principles" and their practical application.
...
This project was the culmination of our learning in Quantitative Methods in Linguistics. The primary growth area was the transition from a frequentist to a Bayesian mindset. Instead of relying on p-values, we learned to interpret model parameters as full probability distributions, quantify uncertainty with credible intervals, and use posterior predictions to understand our model's implications. A particular challenge was understanding how to translate the frequentist contrast coding into a Bayesian framework, but working through this deepened our understanding of how statistical models encode theoretical assumptions. This project solidified our understanding of how to connect abstract linguistic theories, like representational indeterminacy, to concrete, testable statistical models. The hands-on experience of building, validating, and interpreting these models has given us a powerful new toolkit for future research.

### Individual Contribution

*   **Violet Manson:** I was responsible for the primary technical implementation, writing over 800 lines of R code, fitting three distinct Bayesian models (each with 4 chains and 2000 iterations), and generating 14 publication-quality visualizations. I also conducted the model validation and sensitivity analyses to ensure the robustness of our findings.
...
While our analysis provides strong support for the representational indeterminacy hypothesis, future work could extend this approach to other L1-L2 pairings or test the model's predictions with larger sample sizes. Additionally, incorporating reaction time data could provide further insight into the processing mechanisms underlying these effects.
