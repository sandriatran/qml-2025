...
# Title: Task B: Shallow Structure Hypothesis
# Author: Sandria Tran
# Date: 2025-11-6
...
#   (3) Fit a regression model to answer the following question: how does relation type of prime-target affect accuracy in L2 speakers of English?
#   (4) Write a paragraph reporting the model. Produce plots of the posterior distributions of the model parameters and the expected predictions of accuracy for each relation type.
...
# 2. Filter data: L2 speakers only + critical trials ----
shallow_l2_critical <- shallow %>%
  filter(Group == "L2", Critical_Filler == "Critical") %>%
  drop_na(ACC, Relation_type)

cat("Filtered data: L2 speakers with critical trials\n")
cat("N =", nrow(shallow_l2_critical), "observations\n")
cat("Relation types:", unique(shallow_l2_critical$Relation_type), "\n\n")

# 3. Fit Bernoulli regression model ----
# Question: does relation_type affect accuracy in L2 speakers on critical trials?
# Model: ACC ~ Bernoulli(p) where logit(p) = β₀ + β_relation

model_shallow <- brm(
  ACC ~ Relation_type,
  data = shallow_l2_critical,
...
posterior_draws <- as_draws_df(model_shallow)
...
# Plot 2: Relation type effect posteriors
p2 <- posterior_draws %>%
...
p4 <- pp_check(model_shallow, ndraws = 100) +
...
# Plot expected accuracies
p5 <- ggplot(epreds_summary, aes(x = Relation_type, y = mean_acc)) +
  geom_point(size = 3, color = "darkblue") +
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, color = "darkblue") +
  labs(
    title = "Expected Accuracy by Relation Type (L2, Critical Trials)",
    x = "Relation Type",
...
cat("How does relation type of prime-target affect accuracy in L2 speakers\n")
cat("on critical syntactic trials?\n\n")

cat("DATA:\n")
cat("- L2 speakers only\n")
cat("- Critical (not filler) trials\n")
cat("- N =", nrow(shallow_l2_critical), "\n")
cat("- Relation types:", paste(relation_types, collapse = ", "), "\n\n")

cat("MODEL SPECIFICATION:\n")
cat("ACC ~ Bernoulli(p) with logit link\n")
cat("logit(p) = β₀ + β₁·RelationType\n\n")

cat("SUMMARY STATISTICS:\n")
intercept_mean <- mean(posterior_draws$b_Intercept)
cat(sprintf("Intercept (β₀): %.3f log-odds (Prob = %.3f)\n\n",
            intercept_mean, plogis(intercept_mean)))

cat("PREDICTED ACCURACIES BY RELATION TYPE:\n")
print(epreds_summary %>%
        select(Relation_type, mean_acc, lower, upper) %>%
